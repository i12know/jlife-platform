...
- Added `bin/build-reading-plan.js`, a CLI that generates a daily reading plan from the Robertson harmony balanced by Scripture word count, with a choice of `--style=parallel` (read every parallel Gospel account per section) or `--style=primary` (read the longest account only, no repeats) and a `--days=N` target; days never split a section. Backed by `content/harmony/kjv-word-counts.json` (per-verse word counts only, derived from the public-domain KJV via the new `bin/generate-kjv-word-counts.js`) and a coverage check added to `bin/validate-harmony.js`.
- Extended `docs/content-rights.md` §3 with the Thomas & Gundry lettered-subsection question (their NIV/NASB harmonies split Robertson's longer sections into §8a/§8b/§8c-style subsections): recorded that independently choosing our own split points needs no rights clearance, on the same public-domain-arrangement reasoning already applied to Robertson's original 184-section division — only copying their subsection titles/prose or their specific split points verbatim would require it.
- Added `content/harmony/subsections.json`, an additive overlay subdividing 35 of Robertson's 184 sections into lettered pieces: 34 following Thomas & Gundry's NIV/NASB Harmony of the Gospels numbering (cross-verified against two independently sourced transcriptions of their split points before committing), plus one SonLife-only split (§41 trimmed, §44 → §44a/§44b) carrying Luke 5:1-11 out of §41 into its own unit, per SonLife's own teaching tradition rather than Thomas & Gundry's numbering. `robertson-1922-outline.json` itself is untouched. `bin/validate-harmony.js` now validates the new file's structure, parent linkage, letter sequencing, and word-count coverage. `bin/build-reading-plan.js` gained a `--granularity=section|subsection` flag so a reading plan can use the finer-grained units — cuts the worst-day-vs-average spread on a 90-day comparative-harmony plan from 377% to 110% by letting outlier sections (like the Olivet Discourse) split instead of forcing a single oversized day.
- Added `--from`/`--to` (restrict a reading plan to a Robertson section subrange, e.g. Christmas-to-Easter), `--start-date` (map plan days onto real calendar dates), and native `--out=path.csv` export to `bin/build-reading-plan.js`. The CSV writer deliberately uses plain ASCII glue characters (`#`, `-`) rather than the console output's `§`/`—`, on top of a UTF-8 BOM, to survive Excel's habit of misreading a `.csv` with no encoding hint.
...
- `bin/build-reading-plan.js`'s partitioner minimized only the heaviest day's word count, which left it blind once that day hit the floor set by the heaviest indivisible unit — every partition under that ceiling scored identically, so a 20-word day could sit next to a 900-word day at "optimal" cost. Switched to minimizing each day's squared deviation from the target average (least squares), which pulls every day toward the mean instead of only capping the worst one; console output now reports standard deviation rather than min-max spread, and names the indivisible-unit floor when a day is sitting on it.
- Cleaned embedded newlines (an Excel word-wrap artifact from the source spreadsheet) out of 19 `content/harmony/subsections.json` titles, e.g. "Certification of death and procurement of\nbody".
